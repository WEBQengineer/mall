      ...
      <div class="container">
        <div class="cart-box">
          ...
          <ul class="cart-item-list">
            <li class="cart-item" v-for="(item) of list" :key='item.productId'>
    ...
    updateCart(item,type){
      ...
      if (type=='+') {
        if (quantity == item.productStock) {
            this.$message.success('购买商品数量不能超出库存')
            return
        }
        ++quantity
      } else if(type=='-'){
        if (quantity == 1) {
          this.$message.success('商品最小数量为1')
          return
        }
        --quantity
      } else {
